...
export default function QuizSection({ questions, topicTitle = "Understanding Check" }: QuizSectionProps) {
  ...
  return (
    <div id="quiz-section-container" className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col justify-between h-full">
      <div>
        {/* Quiz Header */}
        <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-semibold text-slate-900 tracking-tight text-sm md:text-base">{topicTitle}</h3>
              <p className="text-[9px] text-slate-400 font-mono uppercase tracking-widest font-bold">Comprehension Drills</p>
            </div>
          </div>
        ...
        {quizFinished ? (
          <div className="text-center py-6">
            <Award className="w-10 h-10 text-slate-400 mx-auto mb-3 animate-pulse" />
            <h4 className="text-base font-bold text-slate-950">Quiz Completed!</h4>
            <p className="text-slate-500 text-xs mt-1 mb-5">
              Let's evaluate your interaction score. You scored:
            </p>

            <div className="inline-flex items-center justify-center p-4 bg-slate-50 rounded-2xl border border-slate-200 mb-6">
              <span className="text-3xl font-extrabold text-slate-900">{score}</span>
              <span className="text-slate-400 text-lg mx-1.5">/</span>
              <span className="text-xl font-bold text-slate-500">{questions.length}</span>
            </div>

            <p className="text-xs text-slate-500 italic max-w-sm mx-auto mb-6">
            ...
            <button
              onClick={restartQuiz}
              id="quiz-btn-restart"
              className="px-5 py-2.5 bg-slate-900 hover:bg-slate-850 text-white rounded-xl text-xs font-bold transition-all inline-flex items-center gap-1.5 cursor-pointer"
            >
          ...
          <div className="space-y-4">
            {/* Question Text */}
            <h4 className="font-semibold text-stone-800 text-sm md:text-base leading-snug">
            ...
            <div className="space-y-2 mt-3">
              {activeQuestion.options.map((option, idx) => {
                ...
                if (isSubmitted) {
                  ...
                  optionStyle = 'border-slate-900 bg-slate-50 text-slate-950 font-medium';
                }
              ...
              {isSubmitted && (
                ...
                >
                  <div className="flex items-center gap-1.5 font-bold mb-1.5 uppercase font-mono tracking-wider text-[10px]">
                    <Lightbulb className="w-4 h-4 shrink-0 text-slate-400" />
            ...
            <div className="flex justify-end pt-3 mt-4 border-t border-slate-200">
              {!isSubmitted ? (
                <button
                  ...
                  id="quiz-btn-submit"
                  className="px-4 py-2 bg-slate-900 disabled:opacity-40 disabled:cursor-not-allowed text-white hover:bg-slate-850 leading-none py-2.5 rounded-xl text-xs font-semibold select-none flex items-center gap-1 cursor-pointer"
                >
                  Confirm Answer
                </button>
              ) : (
                <button
                  onClick={handleNext}
                  id="quiz-btn-next"
                  className="px-4 py-2 bg-slate-900 hover:bg-slate-850 text-white leading-none py-2.5 rounded-xl text-xs font-semibold flex items-center gap-1 cursor-pointer"
                >
      ...
      <div className="pt-4 border-t border-slate-100 text-[10px] text-slate-400 font-mono text-left mt-6">
