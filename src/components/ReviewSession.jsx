...
export default function ReviewSession({ exercises, studentId, onClose }) {
  ...
  return (
    <div className="review-overlay modal-overlay-enter" onClick={handleBackdropClick}>
      <div className="review-modal modal-card-enter">
        <div className="review-modal-header">
          <div>
            <div className="review-modal-kicker">Spaced Review</div>
            <div className="review-modal-title">
              {finished ? 'Review complete!' : `Item ${idx + 1} of ${exercises.length}`}
            </div>
          </div>
          <button onClick={onClose} className="review-modal-close" aria-label="Close">
            <Icon.close size={14} />
          </button>
        </div>
