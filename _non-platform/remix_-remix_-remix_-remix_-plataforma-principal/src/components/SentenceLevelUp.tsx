...
export default function SentenceLevelUp({ unitId }: SentenceLevelUpProps) {
  ...
  if (!currentChallenge) {
    return (
      <div className="bg-white rounded-2xl border border-stone-205 border-stone-200 p-8 text-center text-stone-400">
        <Sparkles className="w-10 h-10 text-stone-300 mx-auto mb-3" />
        <p className="font-medium text-stone-500">No Level-Up challenge available for this unit yet.</p>
      </div>
    );
  }
  ...
  const getComparativeTextByLevel = () => {
    if (comparativeLevel === 'B1') {
      return {
        ...
        color: 'text-stone-600 bg-stone-50 border-stone-300',
    ...
    return {
      ...
      color: 'text-orange-900 bg-orange-50/50 border-orange-300',
  ...
  return (
    <div id="sentence-levelup-studio" className="bg-white rounded-2xl border border-stone-205 border-stone-200 p-6 flex flex-col justify-between h-full shadow-3xs">
      
      <div>
        {/* Module Header */}
        <div className="flex flex-wrap items-center justify-between border-b border-stone-100 pb-4 mb-6">
          <div className="flex items-center gap-2.5">
            <div className="p-2.5 bg-stone-900 text-white rounded-full">
              <TrendingUp className="w-4 h-4 text-emerald-300 animate-pulse" />
            </div>
            <div>
              <h3 className="font-semibold text-stone-1000 tracking-tight text-sm md:text-base text-stone-900 font-serif">
                🚀 Sentence Level-Up Studio
              </h3>
              <p className="text-[9px] text-stone-400 font-mono uppercase tracking-widest font-extrabold">
          ...
          {activeChallenges.length > 1 && (
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-stone-400 uppercase tracking-widest font-bold">Challenge Selector:</span>
                  ...
                  <button
                    ...
                    className={`w-7 h-7 rounded-lg text-xs font-bold font-mono transition-all flex items-center justify-center cursor-pointer ${
                      activeChallengeIdx === idx
                        ...
                        : 'bg-stone-50 hover:bg-stone-150 border border-stone-200 text-stone-600'
                    }`}
                  >
                    {idx + 1}
                  </button>
        ...
        <div className="mb-8 bg-stone-50/50 rounded-2xl border border-stone-200/80 p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <span className="text-[9px] font-mono text-stone-400 uppercase tracking-widest font-extrabold block">Comparative Concept Deck</span>
              <h4 className="text-xs font-bold text-stone-700 font-sans font-serif">
                Observe the progression of complexity and polish:
              </h4>
            </div>

            {/* Quick-toggle Pills */}
            <div className="flex bg-stone-100/80 p-1 rounded-xl border border-stone-200/40 text-xs">
          ...
          >
            <div className="flex items-start gap-4 justify-between">
              ...
              <div className="flex gap-1.5 shrink-0">
                <button
                  ...
                  className="p-1 rounded bg-white border border-stone-200 hover:bg-stone-105 hover:text-stone-900 transition-colors cursor-pointer"
            ...
            <p className="font-semibold text-stone-800 text-sm md:text-base leading-relaxed mt-3">
              "{activeCompText.text}"
            </p>
          </motion.div>

          <p className="text-[11px] text-stone-400 mt-2.5 font-sans italic">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div className="lg:col-span-5 flex flex-col justify-between p-4 rounded-2xl border border-stone-200 bg-stone-50/50">
            <div>
              <div className="flex items-center gap-1.5 mb-3 border-b border-stone-200/50 pb-2">
                <HelpCircle className="w-4 h-4 text-stone-500" />
                <span className="font-bold text-xs font-mono uppercase tracking-widest text-stone-400">Upgrade Challenge Quiz</span>
              </div>

              <span className="text-[10px] text-orange-600 font-mono uppercase tracking-wider font-extrabold block mb-1">THE MISSION:</span>
              <p className="text-xs text-stone-650 leading-relaxed font-medium mb-4 text-stone-800">
              ...
              <div className="space-y-2.5">
                {currentChallenge.options.map((opt, oIdx) => {
                  ...
                  let cardStyle = 'border-stone-200 bg-white hover:bg-stone-50';
                  ...
                  if (quizSubmitted) {
                    if (isCorrect) {
                      ...
                      cardStyle = 'border-stone-100 bg-stone-50/30 text-stone-450 opacity-60 cursor-not-allowed';
                    }
                  } else if (isSelected) {
                    cardStyle = 'border-stone-900 bg-stone-100 text-stone-950 font-medium ring-1 ring-stone-900';
                  }
                    ...
                    >
                      <span className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-mono font-bold mt-0.5 border ${
                        quizSubmitted && isCorrect
                          ...
                          : 'bg-white border-stone-300 text-stone-500 animate-none'
                      }`}>
              ...
              <AnimatePresence>
                {quizSubmitted && (
                  <motion.div
                    ...
                    className="p-3 bg-white rounded-xl border border-stone-200 mt-4 text-[11px] leading-relaxed select-text"
                  >
                    <div className="font-bold text-stone-800 mb-1 flex items-center gap-1 uppercase font-mono tracking-wider text-[9px]">
            ...
            <div className="flex justify-end pt-3 mt-4 border-t border-stone-200/60">
              {!quizSubmitted ? (
                <button
                  onClick={() => setQuizSubmitted(true)}
                  disabled={selectedOption === null}
                  className="px-4 py-2 bg-stone-900 disabled:opacity-40 disabled:cursor-not-allowed text-white hover:bg-stone-820 rounded-xl text-xs font-semibold flex items-center gap-1 cursor-pointer transition-colors"
                >
                  Confirm Choice
                </button>
              ) : (
                <button
                  ...
                  className="px-4 py-2 bg-white hover:bg-stone-50 border border-stone-200 text-stone-700 rounded-xl text-xs font-semibold flex items-center gap-1.5 cursor-pointer transition-colors"
                >
          ...
          <div className="lg:col-span-7 flex flex-col justify-between p-4 rounded-2xl border border-stone-200 bg-white">
            <div>
              <div className="flex items-center justify-between mb-3 border-b border-stone-100 pb-2">
                <div className="flex items-center gap-1.5">
                  <Sparkles className="w-4 h-4 text-emerald-500 animate-spin-slow" />
                  <span className="font-bold text-xs font-mono uppercase tracking-widest text-stone-400">Transformation Sandbox</span>
                </div>
                <span className="text-[10px] text-stone-400 italic">Test your own composition skills!</span>
              </div>

              {/* Goal setup banner info */}
              <div className="mb-4 text-xs text-stone-600 bg-stone-50 p-3 rounded-xl border border-stone-200/60 leading-normal">
                <strong>Upgrade Objective:</strong> {currentChallenge.b2Goal}
              </div>

              {/* Interactive target keyword tracker badges */}
              <div className="space-y-2 mb-4">
                <span className="text-[9px] font-mono text-stone-400 uppercase tracking-widest font-extrabold block">Include these advanced keywords to light up the badges:</span>
                    ...
                    return (
                      ...
                      >
                        <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold text-[9px] ${
                          isMatched ? 'bg-emerald-500 text-white' : 'bg-stone-100 text-stone-400 border border-stone-300/40'
                        }`}>
                          {isMatched ? '✓' : idx + 1}
                        </span>
              ...
              <div className="relative">
                <textarea
                  ...
                  className="w-full h-[110px] p-3 text-xs md:text-sm bg-stone-50 rounded-xl border border-stone-200 focus:outline-none focus:ring-1 focus:ring-stone-400 text-stone-805 leading-relaxed placeholder:text-stone-400 resize-none font-sans font-normal"
                />
                ...
                <AnimatePresence>
                  {allKeywordsMatched && (
                    ...
                    >
                      ...
                      <h5 className="font-bold text-sm font-serif">Perfect Upgrade Link Crafted!</h5>
                      ...
                      <button
                        ...
                        className="px-3 py-1 bg-white hover:bg-stone-100 text-stone-900 rounded-lg text-[10px] font-bold font-mono transition-colors uppercase tracking-widest cursor-pointer"
                      >
                        Try alternative
                      </button>
                    </motion.div>
              ...
              <div className="mt-4 bg-stone-50/50 rounded-xl border border-stone-200 p-3 text-[11px] leading-relaxed">
                <span className="font-bold text-stone-700 uppercase font-mono text-[9px] tracking-wider block mb-1">Structural Cheat Sheet:</span>
                <p className="text-stone-600">
                  💡 <strong>Inversion Trigger:</strong> {currentChallenge.sentenceInversionHint}
                </p>
                
                <div className="mt-2.5 space-y-1 bg-white p-2 rounded border border-stone-150">
                  <span className="font-semibold text-stone-400 text-[9px] uppercase font-mono tracking-widest block">Lexicon Upgrade Explainer:</span>
                  {currentChallenge.targetKeywords.map((k, i) => (
                    <div key={i} className="text-[10px] text-stone-550 leading-relaxed text-stone-600 flex items-start gap-1">
              ...
              <AnimatePresence>
                {showModelAnswers && (
                  <motion.div
                    ...
                    className="mt-4 p-3 bg-orange-50/40 border border-orange-100 rounded-xl text-xs space-y-3"
                  >
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-bold font-mono text-[10px] text-orange-800 uppercase tracking-wider block">Model B2 Target Sentence:</span>
                        <button
                          onClick={() => handleCopyText(currentChallenge.options[currentChallenge.correctOptionIndex].text, 'b2')}
                          className="text-[9px] font-mono font-bold text-orange-600 uppercase tracking-widest bg-white border border-orange-200 px-2 py-0.5 rounded flex items-center gap-1 hover:bg-stone-100 cursor-pointer"
                        >
                          {copiedLevel === 'b2' ? 'Copied ✓' : 'Copy'}
                        </button>
                      </div>
                      <p className="text-stone-800 italic font-medium">"{currentChallenge.options[currentChallenge.correctOptionIndex].text}"</p>
                    </div>

                    <div className="border-t border-orange-150 pt-2">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-bold font-mono text-[10px] text-orange-800 uppercase tracking-wider block">C1 Advanced Target Alternative:</span>
                        <button
                      ...
                      <p className="text-stone-800 italic font-medium">"{currentChallenge.c1AdvancedAlternative}"</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="flex justify-between items-center pt-3 mt-4 border-t border-stone-100">
              <button
                ...
                className="text-[10px] font-bold font-mono text-orange-605 text-orange-700 hover:text-orange-900 uppercase tracking-wider cursor-pointer font-sans"
              >
              ...
              <button
                ...
                className="px-3.5 py-1.5 bg-stone-50 hover:bg-stone-100 border border-stone-200 rounded-lg text-[10px] font-bold font-mono text-stone-600 uppercase tracking-wider cursor-pointer"
              >
                Insert Guide Draft
              </button>
            </div>
      ...
      {/* Footer statistics */}
      <div className="pt-4 border-t border-stone-100 text-[9px] text-stone-400 font-mono tracking-wider flex items-center justify-between uppercase font-bold mt-6 select-none">
