...
export default function DialoguePlayer({ roleMode, setRoleMode, onSelectWord, script }: DialoguePlayerProps) {
  ...
  const [viewLayout, setViewLayout] = useState<'unified' | 'split'>('split');
  ...
  return (
    <div id="dialogue-player-section" className="bg-white rounded-2xl border border-stone-200 p-6 shadow-xs flex flex-col h-full justify-between">
      ...
      <div>
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-stone-100 mb-6">
          <div className="flex items-center gap-2.5">
            <div className="p-2.5 bg-stone-50 rounded-full text-stone-600 border border-stone-200/50">
              <Radio className="w-4 h-4" />
            </div>
            <div>
              <h3 className="font-semibold text-stone-900 tracking-tight text-base font-serif">Audio Sandbox</h3>
              <p className="text-[10px] text-stone-400 font-mono uppercase tracking-widest">Interactive Dialogue Practice</p>
            </div>
          </div>
          ...
          {isAudioSupported && (
            <div className="flex items-center gap-3 text-xs text-stone-600">
              <div className="flex flex-col gap-1">
                <label className="font-bold text-[9px] text-stone-400 uppercase tracking-widest font-mono">Host / Speaker A</label>
                <select
                  id="voice-interviewer-select"
                  value={selectedInterviewerVoice}
                  onChange={(e) => setSelectedInterviewerVoice(e.target.value)}
                  className="bg-stone-50 border border-stone-200 rounded px-2.5 py-1 text-[11px] focus:outline-none focus:ring-1 focus:ring-stone-400 max-w-[130px] md:max-w-[150px] text-ellipsis cursor-pointer"
                >
                  {voices.map(v => (
              ...
              <div className="flex flex-col gap-1">
                <label className="font-bold text-[9px] text-stone-400 uppercase tracking-widest font-mono">Guest / Speaker B</label>
                <select
        ...
        <div className="bg-stone-50/50 p-1 rounded-xl border border-stone-200/80 mb-6 flex flex-col md:flex-row items-center justify-between gap-3">
          <span className="text-[10px] font-bold text-stone-400 px-2.5 font-mono uppercase tracking-widest hidden md:inline">Oral Practice</span>
        ...
        <div className="text-xs text-stone-500 bg-stone-50 p-3.5 rounded-xl flex flex-col gap-2.5 mb-6 border border-stone-200/60 leading-relaxed font-sans">
          ...
          {roleMode === 'interactive_dialogue' && currentLineIndex === null && (
            <div className="mt-2 pt-2.5 border-t border-stone-200/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3 select-none">
              <div className="flex items-center gap-2">
                <span className="font-bold text-[9px] text-stone-400 font-mono uppercase tracking-wider">Choose Your Character Role:</span>
                  ...
                  {Array.from(new Set(script.map(line => line.speaker))).map(sp => (
                    <button
                      ...
                      className={`text-[10px] font-bold px-2.5 py-1 rounded-md transition-all cursor-pointer ${
                        selectedCharacter === sp ? 'bg-orange-650 text-white bg-orange-600' : 'text-stone-500 hover:text-stone-800'
                      }`}
                    >
                      {sp}
                    </button>
              ...
              <button
                ...
                className="bg-orange-600 hover:bg-orange-750 text-white text-[11px] font-bold px-3 py-1.5 rounded-lg flex items-center justify-center gap-1 transition-colors cursor-pointer self-start sm:self-auto"
              >
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex bg-white rounded-lg p-0.5 border border-stone-200">
                  <button
                    ...
                    className={`text-[10px] font-bold px-2.5 py-1 rounded-md transition-all cursor-pointer ${
                      roleplayTarget === 'A' ? 'bg-stone-900 text-white' : 'text-stone-500 hover:text-stone-800'
                    }`}
                  >
                    Speaker A (Host)
                  </button>
                  <button
                    ...
                    className={`text-[10px] font-bold px-2.5 py-1 rounded-md transition-all cursor-pointer ${
                      roleplayTarget === 'B' ? 'bg-stone-900 text-white' : 'text-stone-500 hover:text-stone-800'
                    }`}
                  >
                    Speaker B (Guest)
                  </button>
        ...
        <div className="bg-stone-50/50 p-2.5 rounded-xl border border-stone-200/60 mb-5 flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-1.5">
            <span className="font-semibold text-stone-700 font-serif">Transcript Mode</span>
            <span className="text-[9px] bg-stone-200/60 text-stone-500 font-mono font-bold px-1.5 py-0.5 rounded">V2</span>
          </div>
          <div className="flex bg-stone-200/50 p-0.5 rounded-lg border border-stone-200/30">
            <button
              ...
              className={`px-3 py-1 text-[11px] font-medium rounded-md transition-all flex items-center gap-1.5 cursor-pointer ${
                viewLayout === 'unified'
                  ? 'bg-white text-stone-900 shadow-3xs border border-stone-200/25 font-semibold'
        ...
        <div id="dialogue-scroll-container" className="space-y-6 max-h-[420px] overflow-y-auto pr-2 custom-scrollbar scroll-smooth relative p-1">
          {viewLayout === 'split' && (
            <div className="hidden md:block absolute left-1/2 top-4 bottom-4 w-0.5 bg-stone-100 -translate-x-1/2 pointer-events-none" />
          )}

          {script.map((line, idx) => {
            const isPrimary = line.speaker === 'Interviewer' || 
                              line.speaker === 'Host' || 
                              line.speaker === 'Agent' || 
                              line.speaker === 'Tourist' || 
                              line.speaker === 'A' || 
                              line.speaker === 'Organizer' || 
                              line.speaker === 'Announcer' ||
                              line.speaker === 'Receptionist' ||
                              line.speaker === (script[0]?.speaker); // First speaker fallback
            ...
            // Speak initials
            const avatarInitials = line.speaker.slice(0, 2).toUpperCase();

            // Premium colored avatar + card details
            const dialogueLineContent = (
              <div 
                className={`group flex items-start gap-3.5 transition-all duration-300 w-full ${
                  isPrimary ? 'flex-row' : 'flex-row-reverse'
                }`}
              >
                {/* Speaker Avatar indicator with premium colors */}
                <div className={`shrink-0 w-11 h-11 rounded-full flex items-center justify-center font-bold text-[11px] tracking-tight uppercase border transition-all duration-300 ${
                  isPrimary
                    ? (isActive 
                        ? 'bg-indigo-600 text-white border-indigo-700 shadow-md scale-105 ring-4 ring-indigo-100/50' 
                        : 'bg-indigo-50 text-indigo-700 border-indigo-150 hover:bg-indigo-100/70') 
                    : (isActive 
                        ? 'bg-teal-600 text-white border-teal-700 shadow-md scale-105 ring-4 ring-teal-100/50' 
                        : 'bg-teal-50 text-teal-700 border-teal-150 hover:bg-teal-100/70')
                } relative`}>
                  {isActive && (
                    <span className={`absolute -inset-1 rounded-full animate-ping border opacity-20 ${
                      isPrimary ? 'border-indigo-400' : 'border-teal-400'
                    }`} />
                  )}
                  {avatarInitials}
                </div>

                {/* Bubble card with subtle speaker background coloring */}
                <div className={`grow max-w-[85%] rounded-2xl p-4 transition-all duration-350 border ${
                  isActive 
                    ? (isPrimary 
                        ? 'bg-indigo-50/40 border-indigo-300 shadow-sm ring-1 ring-indigo-300/30' 
                        : 'bg-teal-50/40 border-teal-300 shadow-sm ring-1 ring-teal-300/30')
                    : (isPrimary 
                        ? 'border-stone-150 bg-indigo-50/10 hover:border-indigo-200/50 hover:bg-indigo-50/20' 
                        : 'border-stone-150 bg-teal-50/10 hover:border-teal-200/50 hover:bg-teal-50/20')
                }`}>
                  <div className="flex items-center justify-between gap-2 mb-1.5">
                    <span className={`text-[10px] uppercase tracking-wider font-extrabold ${
                      isPrimary ? 'text-indigo-600' : 'text-teal-700'
                    }`}>
                      {line.speaker}
                    </span>
                    ...
                    <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                      {isObscured && (
                        <button
                          onClick={(e) => { e.stopPropagation(); toggleRevealLine(line.id); }}
                          className="p-1 rounded text-stone-400 hover:text-stone-700 hover:bg-stone-150 transition-colors cursor-pointer"
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); speakSpecificLine(idx); }}
                        className={`p-1 rounded hover:bg-stone-150 transition-colors cursor-pointer ${
                          isActive ? 'text-stone-800 bg-stone-200/30' : 'text-stone-400 hover:text-stone-700'
                        }`}
                  ...
                  <div className="text-stone-800 leading-relaxed text-sm md:text-base font-sans font-normal">
                    {roleMode === 'interactive_dialogue' && isActive && line.speaker === selectedCharacter ? (
                      <div className="space-y-4 mt-2">
                        {/* Word pronunciation teacher title */}
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold text-orange-650 text-orange-600 uppercase tracking-wider font-mono flex items-center gap-1.5">
                        ...
                        <div className="flex flex-wrap gap-1.5 p-3.5 bg-stone-100/55 rounded-xl border border-stone-200/50">
                          {line.text.split(/\s+/).map((word, wIdx) => {
                            let badgeStyle = "bg-white text-stone-800 hover:bg-stone-200 border-stone-200 font-sans";
                            ...
                            if (speechMetrics) {
                              ...
                              if (matchObj) {
                                if (matchObj.status === 'perfect') {
                                  ...
                                  badgeStyle = "bg-rose-100 text-rose-800 border-rose-300 hover:bg-rose-200 font-sans";
                                }
                              }
                            }
                        ...
                        {speechMetrics && (
                          <motion.div 
                            ...
                            className="bg-orange-50/40 p-4 rounded-xl border border-orange-100/60 flex flex-col gap-3.5"
                          >
                            <div className="grid grid-cols-2 gap-3.5 animate-fade-in font-sans">
                              {/* circular design match scoring */}
                              <div className="bg-white rounded-lg p-3 border border-stone-200/50 flex flex-col items-center justify-center text-center">
                                <div className="text-base font-bold text-orange-700 tracking-tight font-mono">{speechMetrics.accuracy}%</div>
                                <div className="text-[10px] uppercase tracking-wider font-bold text-stone-400 mt-0.5">Pronunciation Match</div>
                              </div>
                              ...
                              <div className="bg-white rounded-lg p-3 border border-stone-200/50 flex flex-col items-center justify-center text-center">
                                ...
                                <div className="text-[10px] uppercase tracking-wider font-bold text-stone-400 mt-0.5 font-sans">Pacing tempo</div>
                              </div>
                            </div>

                            {/* comparative align outputs */}
                            <div className="font-sans">
                              <div className="text-[10px] uppercase tracking-wider font-bold text-stone-400 font-mono mb-1">What we heard:</div>
                              <p className="text-xs text-stone-700 italic bg-white/70 p-2.5 rounded border border-stone-200/50 leading-relaxed font-sans">
                                "{spokenTranscript || '...'}"
                              </p>
                              <p className="text-[10px] text-orange-655 font-medium mt-1.5 flex items-center gap-1 font-sans">
                                <Info className="w-3.5 h-3.5 shrink-0 text-orange-600" /> {speechMetrics.pacingFeedback}
                              </p>
                            </div>
                          </motion.div>
                        )}
                        ...
                        <div className="flex items-center gap-2 font-sans">
                          {!isRecording ? (
                            <button
                              ...
                              className="grow bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer shadow-xs hover:shadow-sm"
                            >
                              <Mic className="w-4 h-4 fill-white animate-pulse" /> Start Recording Line
                            </button>
                          ) : (
                            <button
                              onClick={(e) => { e.stopPropagation(); stopSpeechRecognition(); }}
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); simulateRecording(); }}
                            disabled={isRecording}
                            className="bg-stone-100 hover:bg-stone-200 border border-stone-200 text-stone-700 text-xs font-bold py-2.5 px-3 rounded-xl transition-all cursor-pointer disabled:opacity-40"
                      ...
                      <div className="space-y-3 mt-1.5 select-none">
                        <div className="flex items-center gap-1.5 text-[10px] font-bold text-orange-600 uppercase tracking-wider font-mono">
                          <Check className="w-3.5 h-3.5 text-orange-500" /> Choose the B2 Upgraded phrasing:
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          {roleplayChoices.map((choice, cIdx) => {
                            ...
                            let borderStyle = "border-stone-200 bg-white hover:border-stone-400 hover:bg-stone-50";
                            let iconBadge = null;
                            ...
                            if (roleplaySubmitted) {
                              ...
                              borderStyle = "border-orange-500 bg-orange-50/40 font-medium text-stone-900";
                            }
                            ...
                            return (
                              <button
                                ...
                                onClick={(e) => { e.stopPropagation(); setSelectedRoleplayChoice(cIdx); }}
                        ...
                        {!roleplaySubmitted ? (
                          <button
                            onClick={(e) => { e.stopPropagation(); handleRoleplaySubmit(); }}
                            ...
                            className={`w-full py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-1 cursor-pointer ${
                              selectedRoleplayChoice !== null
                                ? 'bg-orange-600 hover:bg-orange-700 text-white hover:shadow-xs'
                          ...
                          roleplayChoices[selectedRoleplayChoice!]?.type !== 'correct' && (
                            <button
                              ...
                              className="w-full py-2 bg-stone-900 hover:bg-stone-800 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer"
                            >
                              Try Another Choice
                            </button>
                          )
                      ...
                      isObscured && !isRevealed ? (
                        <motion.div 
                          ...
                          onClick={(e) => { e.stopPropagation(); toggleRevealLine(line.id); }}
                        >
                          <span className="font-mono text-[11px] text-stone-300 select-none tracking-widest font-bold">
                            •••••••••••• •••••••••••• ••••••••••••
                          </span>
                          <span className="text-[9px] uppercase font-mono tracking-wider bg-stone-100 text-stone-500 px-2.5 py-1 rounded shrink-0 flex items-center gap-1 group-hover:bg-stone-200 transition-colors">
            ...
            // Row wrapper based on active view layout
            if (viewLayout === 'split') {
              return (
                <div 
                  key={line.id}
                  ref={isActive ? activeLineRef : null}
                  className={`grid grid-cols-1 md:grid-cols-12 gap-4 items-start transition-all duration-300 ${
                    isActive ? 'opacity-100' : currentLineIndex !== null ? 'opacity-40 hover:opacity-95' : 'opacity-100'
                  }`}
                >
                  {isPrimary ? (
                    <>
                      <div className="md:col-span-6 flex justify-start w-full">{dialogueLineContent}</div>
                      <div className="hidden md:block md:col-span-6" />
                    </>
                  ) : (
                    <>
                      <div className="hidden md:block md:col-span-6" />
                      <div className="md:col-span-6 flex justify-end w-full">{dialogueLineContent}</div>
                    </>
                  )}
                </div>
              );
      ...
      <div className="mt-8 pt-5 border-t border-stone-100 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="xl:min-w-[140px] flex items-center gap-2">
          <span className="text-xs text-stone-400 font-mono uppercase tracking-wider font-semibold">Speed:</span>
              ...
              <button
                ...
                className={`text-[10px] font-mono px-2.5 py-1 rounded-full border cursor-pointer transition-colors ${
                  playbackRate === rate 
                    ...
                    : 'bg-white border-stone-200 text-stone-655 hover:bg-stone-50'
                }`}
              >
                {rate}x
              </button>
        ...
        {currentLineIndex !== null && (
          <div className="text-xs font-mono text-stone-400 uppercase tracking-widest font-semibold">
            Line <span className="text-stone-900 font-bold">{currentLineIndex + 1}</span> of <span className="text-stone-900 font-bold">{script.length}</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            id="btn-global-reset"
            className="p-2 rounded-full text-stone-400 hover:text-stone-700 hover:bg-stone-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer border border-transparent hover:border-stone-200/60"
          ...
          <button
            ...
            className={`px-5 py-2 rounded-full font-medium shadow-xs flex items-center gap-2 text-xs uppercase tracking-widest transition-all cursor-pointer ${
              isPlaying
                ? 'bg-stone-800 hover:bg-stone-900 text-white font-bold'
                : 'bg-stone-900 hover:bg-stone-850 text-white font-semibold'
            }`}
          >
            {isPlaying ? (
              <>
          ...
          {roleMode !== 'all' && currentLineIndex !== null && (
            <button
              ...
              className="px-4 py-2 bg-white hover:bg-stone-50 border border-stone-200 rounded-full text-stone-800 font-semibold text-xs uppercase tracking-wider flex items-center gap-1 transition-all cursor-pointer"
            >
